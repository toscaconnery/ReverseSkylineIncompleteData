...
def generate_query_point(): #NEW
	global query_point
	query_point = "QP 30 30 30 30"
	#query_point = "QP 45 45 45 45"
	#query_point = "QP 70 70 70 70"

	#query_point = "QP 18 20 18 20"
...
def naive_algorithm(new_ct, new_q):
	global data_length
	global loop_status
	global maximum
	global minimal
	global skyline_combination
	global candidate_combination
	global pivot
	global q_cost
	global ct_cost
	global query_point
	...
	maximum = list(new_q[1:data_length+1])
	minimal = list(new_ct)
	pivot = list(new_ct)
	...
	while(loop_status == True):
		...
		# print(pivot)
		a = [list(pivot), list(maximum)]
		candidate_combination.append(a)

	#check_is_skyline(pivot, q)
	for i in range(0, len(candidate_combination)):
		#print("MMMMM" + str(candidate_combination[i]))
		check_is_skyline(candidate_combination[i][0], candidate_combination[i][1])
	#check apakah ct baru dengan nilai pivot akan menjadikannya sebagai skyline
	cheapest_index =  None
	cheapest_cost = 99999999
	if(len(skyline_combination) > 0):
		# print("ADA KEMUNGKINAN")
		for data_index in range(0, len(skyline_combination)):
			current_cost = 0
			# print("SKYLINE COMBINATION Q  : ")
			# print(skyline_combination[data_index]['q'])
			# print("SKYLINE COMBINATION CT : ")
			# print(skyline_combination[data_index]['ct'])
			# print("REAL Q                 : ")
			# print(q)
			# print("REAL CT                : ")
			# print(ct)
			...
			# print(ct_cost)
			for i in range(0, data_length):
				current_cost += ((abs(q[i] - skyline_combination[data_index]['q'][i])) * q_cost[i])
				current_cost += ((abs(ct[i] - skyline_combination[data_index]['ct'][i])) * ct_cost[i])
			if(current_cost < cheapest_cost):
				cheapest_cost = current_cost
				cheapest_index = data_index
	# print("Jumlah kemungkinan : " + str(len(skyline_combination)))

	print("CHEAPEST RECOMMENDATION : ")
	print("Q  : " + str(skyline_combination[cheapest_index]['q']))
	print("CT : " + str(skyline_combination[data_index]['ct']))
...
def check_is_skyline(pivot, q):
	...
	# q = query_point.split()
	# print("Q : " + str(q))
	transformed_query_point = list(q)
	# print("TRANSFORMED Q : " + str(transformed_query_point))
	for i in range(0, data_length):
		transformed_query_point[i] = abs(float(transformed_query_point[i]) - pivot[i])

	# print("SAL")
	# print("Q     : " + str(transformed_query_point))
	# print("PIVOT : " + str(pivot))

	q_is_skyline = True
	# print("LEN GLOBAL : " + str(len(global_skyline)))
	...
	if(len(global_skyline) > 0):
		for data_index in range(0, len(global_skyline)):
			...
			greater = False
			for i in range(0, data_length):
				if(transformed_query_point[i] < pivot[i]):
					smaller = True
				elif(transformed_query_point[i] > pivot[i]):
					greater = True
