...
def Insert_Local_Skyline(current_specs, current_bit):
	...
	for i in range(0, len(local_skyline[current_bit])):	#pengulangan sebanyak data yang ada didalam local_skyline, untuk dibandingkan satu persatu dengan data baru
		...
		print("current_specs : " + str(current_specs))
		for j in range(data_length + 1, data_length * 2 + 1):
...
def Update_Global_Skyline():
	...
	for c in range(0, len(candidate_skyline)):
		for g in range(0, len(global_skyline)):
			...
			dominating_candidate = False
			for i in range(data_length + 1, data_length * 2 + 1):
...
for x in range(0, len(user_preference[0])):
	...
	for line in fp:
		...
		data_length = len(current_specs) - 1
		product_data = list(current_specs)
		print("CS : " + str(current_specs))
		for i in range(1, data_length + 1):
			if(current_specs[i] == "null"):
				current_bit += "0"
				product_data.append("null")
			else:
				current_bit += "1"
				product_data.append(abs(int(current_specs[i]) - user_preference[i-1][x]))
				#current_specs[i] = abs(int(current_specs[i]) - user_preference[i][x])
		if current_bit not in node:
			node[current_bit] = []
			node[current_bit].append(product_data)
		...
		is_skyline = Insert_Local_Skyline(product_data, current_bit)
		if is_skyline == True:
